...
contract BundleBond is RBAC {
    ...
    function addAdmin(string _adminName, address _adminAddress) {
        ...
        if (!t.exists) {
            AdminList[_adminAddress] = Admin({exists: true, name: _adminName});
    ...
    function getAdminName(address _adminAddress) view returns (string) {
    ...
    function isTeamOwner() public returns (bool) {
    ...
    function getTeamOwnerName(address _teamOwnerAddress) view returns (string) {
        TeamOwner storage t = TeamOwnerList[_teamOwnerAddress];
        if (t.exists) {
    ...
    function purchaseWeekOfTeam(uint256 teamTokenId, uint256 weekTokenId)
        public
        payable
    {
        //price to team
    ...
    function makeAnOffer(uint256 weekTokenId) public payable {
        //price to contract
        //offer map update
    }

    function acceptAnOffer(uint256 offerID) public payable {
        //price to prev owner
    ...
    function AddProjectToResume(string _name, address _projectAddress) public {
